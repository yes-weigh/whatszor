...
async function runAutoMergeSimulation(
    ...
    console.log({ [logPrefix]: 'BEFORE MERGE' }, JSON.stringify(beforeProduct, null, 2));
        ...
        for (const [key, value] of Object.entries(finalHybridOutputs.specifications)) {
            ...
            if (conf >= 85) {
                if (existingSpecs[key] === undefined || existingSpecs[key] === null || String(existingSpecs[key]).trim() === '') {
                    newSpecs[key] = value;
                    hasMergedAny = true;
                    console.log({ key, value }, `[${logPrefix}] Merging NEW field!`);
                } else {
                    if (String(existingSpecs[key]).trim().toLowerCase() !== String(value).trim().toLowerCase()) {
                        hasConflictAny = true;
                        console.log({ key, existing: existingSpecs[key], new: value }, `[${logPrefix}] CONFLICT detected!`);
                    } else {
                        console.log({ key }, `[${logPrefix}] NO-OP detected (values match perfectly).`);
                    }
                }
            } else {
                 console.log({ key, conf }, `[${logPrefix}] Ignoring field due to low confidence (< 85)`);
            }
        }
    }
    ...
    console.log({ [logPrefix]: 'AFTER MERGE (Product)' }, JSON.stringify(afterProduct, null, 2));
    console.log({ [logPrefix]: 'AFTER MERGE (Source Status)' }, afterSource?.status);
}

async function main() {
    console.log('--- TESTING PHASE 5: AUTO-MERGE ENGINE ---');
